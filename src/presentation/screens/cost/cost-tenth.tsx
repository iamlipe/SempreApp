...
export function CostTenth() {    
    ...
    const onSubmit = async () => {
        handleData({ idiomaDaDocumentacao, tipoDeDocumentacao, ensaiosETestes })
        setCanGo(true)
    }

    useEffect(() => {
        setIdiomaDaDocumentacao(data.idiomaDaDocumentacao)
        setTipoDeDocumentacao(data.tipoDeDocumentacao)
        setEnsaiosETestes(data.ensaiosETestes)
    }, [])

    useEffect(() => {
        if(canGo) {
            const fields: (keyof ProposalData)[] = ['idiomaDaDocumentacao', 'tipoDeDocumentacao', 'ensaiosETestes'];
    ...
    return (
        <div className="flex flex-col space-y-2">
            ...
            <Select label="Idioma da Documentação" error={errors.idiomaDaDocumentacao} onChange={(e) => setIdiomaDaDocumentacao(e.target.value)} value={idiomaDaDocumentacao}>
            ...
            <Select label="Tipo de Documentação" error={errors.tipoDeDocumentacao} onChange={(e) => setTipoDeDocumentacao(e.target.value)} value={tipoDeDocumentacao}>
                <option value="">Selecione...</option>
            ...
            <Select label="Ensaios e teste" error={errors.ensaiosETestes} onChange={(e) => setEnsaiosETestes(e.target.value)} value={ensaiosETestes}>
                <option value="">Selecione...</option>
